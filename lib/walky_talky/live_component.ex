defmodule WalkyTalky.LiveComponent do
  ...
  def mount(socket) do
    ...
    {:ok, socket |> assign(:walky_talky_local, []) |> assign(:transition, transition)}
  end

  def update(assigns, %{assigns: %{walky_talky_local: walky_talky_local}} = socket) do
    socket =
      socket
      |> assign(assigns)
      |> assign(:walky_talky_local, Enum.concat(walky_talky_local, assigns.walky_talky || []))
